...
function _show_schroot_config()
{
    ...
    if _sp.find_program schroot _ssc_schroot_bin; then
        local -r _ssc_total=$(${_ssc_schroot_bin} -l | wc -l)
        local -r _ssc_active=$(${_ssc_schroot_bin} --all-sessions -l 2>/dev/null | wc -l)
